...
def result(course_id, assessment_id):
    ...
    cur1.execute("""SELECT * FROM student WHERE account_id = %s""",
                        (current_user.account_id, ))
    result = cur1.fetchone()
    cur1.close()
    cur2 = mysql.connection.cursor()
    cur2.execute("""SELECT course_title FROM course WHERE course_id = %s""",(course_id,))
    course_data = cur2.fetchone()[0]
    cur2.close()
    cur3 = mysql.connection.cursor()
    cur3.execute("""SELECT assessment_title FROM assessment WHERE assessment_id = %s""",(assessment_id,))
    assessment = cur3.fetchone()[0]
    cur3.close()
    student_obj = Student(result['student_id'], result['firstname'].upper(), result['lastname'].upper(), result['join_date'], result['account_id'])
    return render_template('result.html', user=current_user, marks=round(eval(marks)*100,2), stu=student_obj, ass=assessment, course=course_data)
